    ...
    .action(
      cloudAction(
        async (
          ...
          cloudOutput(servers, formatCloudServerList, options);
        }
      )
    );
    ...
    .action(
      cloudAction(
        async (client, idOrName: string, options: CloudActionOptions) => {
          const id = await resolveIdOrName(idOrName, "server", (name) =>
            client.listServers({ name })
          );
          const srv = await client.getServer(id);
          cloudOutput(srv, formatCloudServerDetails, options);
        }
      )
    );
    ...
    .action(
      cloudAction(
        async (
          ...
          console.log(
            success(
              `Server '${result.server.name}' created (ID: ${result.server.id})`
            )
          );
          if (result.root_password) {
            console.log(
              `Root password: ${colorize(result.root_password, "yellow")}`
            );
          }
          console.log(
            info(`IPv4: ${result.server.public_net.ipv4?.ip || "pending"}`)
          );
        }
      )
    );
  ...
  server
    ...
    .action(
      cloudAction(async (client, idOrName: string) => {
        ...
        console.log(
          `New password: ${colorize(result.root_password, "yellow")}`
        );
      })
    );
    ...
    .action(
      cloudAction(
        async (
          ...
          console.log(
            success(`Server ${id} attached to network ${options.network}.`)
          );
        }
      )
    );
