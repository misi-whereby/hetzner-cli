...
import { formatIpDetails, formatIpList } from "../formatter.js";
  ...
  ip.command("list")
    ...
    .action(
      asyncAction(async (client, options: ActionOptions) => {
        ...
        output(ips, formatIpList, options);
      })
    );
  ...
  ip.command("get <ip>")
    ...
    .action(
      asyncAction(async (client, ipAddr: string, options: ActionOptions) => {
        ...
        output(ipData, formatIpDetails, options);
      })
    );
      ...
      asyncAction(
        async (
          ...
          await client.updateIp(
            ...
            options.hourly ? Number.parseInt(options.hourly, 10) : undefined,
            options.daily ? Number.parseInt(options.daily, 10) : undefined,
            options.monthly ? Number.parseInt(options.monthly, 10) : undefined
          );
          console.log(success(`IP ${ipAddr} updated.`));
        }
      )
    );
  ...
  mac
    ...
    .action(
      asyncAction(async (client, ipAddr: string) => {
        ...
        console.log(success(`Generated MAC: ${macData.mac}`));
      })
    );
  ...
  mac
    ...
    .action(
      asyncAction(async (client, ipAddr: string) => {
        ...
        console.log(success("MAC address deleted."));
      })
    );
}
