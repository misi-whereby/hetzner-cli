...
import { formatSshKeyDetails, formatSshKeyList } from "../formatter.js";
...
export function registerKeyCommands(parent: Command): void {
  ...
  key
    ...
    .action(
      asyncAction(async (client, options: ActionOptions) => {
        ...
        output(keys, formatSshKeyList, options);
      })
    );
  ...
  key
    ...
    .action(
      asyncAction(
        async (client, fingerprint: string, options: ActionOptions) => {
          const { key: keyData } = await client.getSshKey(fingerprint);
          output(keyData, formatSshKeyDetails, options);
        }
      )
    );
    ...
    .action(
      asyncAction(
        async (
          ...
          console.log(success(`SSH key added: ${newKey.fingerprint}`));
        }
      )
    );
  ...
  key
    ...
    .action(
      asyncAction(async (client, fingerprint: string, name: string) => {
        ...
        console.log(success(`SSH key renamed to: ${name}`));
      })
    );
  ...
  key
    ...
    .action(
      asyncAction(
        async (client, fingerprint: string, options: ActionOptions) => {
          if (
            !(await confirmAction(`Delete SSH key ${fingerprint}?`, options))
          ) {
            return;
          }
          await client.deleteSshKey(fingerprint);
          console.log(success("SSH key deleted."));
        }
      )
    );
}
